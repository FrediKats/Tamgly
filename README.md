...
- State - стэйт задачи, открытая они или закрытая. Создаются задачи Open, а после выполнения переводятся они в Closed.
...
- AssignTo - информация о том, на кого назначена задача. Ввиду того, что прототип делается с расчётом на то, что пользователь только одни и он управляет своими задачами, в системе есть возможность указать только заранее добавленных пользователей. Для основного пользователя Id будет всегда Guid.Empty.
...
- Daily - Содержит все задачи, которые нужно сделать в указанный день. Помимо этого, в этот беклоге также собирается информация о суммарном кол-ве эстимейтов на эту неделю и месяц, чтобы можно делать задачи, которые нужно сделать не обязательно сегодня, чтобы не оказалось, что все weekly задачи остались на последний день.
...
Проект - это сущность, которая позволяет категоризировать задачи по различным направлениям деятельности. Помимо фильтров задач по проектам, в будущем планируется реализовать возможность задавать им кастомные настройки. Помимо этого для проекта можно определить Working hours - количество часов, которые выделяется на этот проект в день или неделю. Используя эту информацию для проекта будет считаться когда будет выполнена задача с учётом приоритетов. Или же находить дни/недели, на которых задач запланировано больше чем выделено часов.

## Repetitive work items

RepetitiveWorkItems - это повторяющаяся задача, которая заводится, чтобы не создавать регулярно однотипные задачи, если действие нужно делать с определённой периодичность. Есть несколько видом интервалов периодичности:
...
Для таких задач создаётся парентовая задача, в которой хранится шаблон (например, имя и описание), и на каждый выпавший интервал создаётся чилдовая задача, чтобы можно было в ней трекать стэйт.
...
Выполнение задачи может зависеть от другой задачи. Иными словами, есть задача, которая блокирует другую задачу. Это важное свойство нужно учитывать при попытках подсчитать время выполнения. Время на выполнение задачи нужно считать как время, которое нужно потратить на все блокирующие задачи плюс время на текущую. Для того, чтобы построить зависимости, необходимо построить граф зависимостей задач и посчитать суммарное количество эстимейтов.
...
Одна из поставленных задач - уметь считать когда может быть выполнена задача, которую ещё не добавили учитывая её эстимейты и приоритет. Эта логика построенная на том, что задачи являются упорядоченными по приоритету, значит можно посчитать через какое время будут выполнены все более приоритетные задачи. Также, важно уметь определять какие задачи будут выполнены позже, чем было заявлено, из-за добавления новой задачи.

## UTC and local date

В системе очень много моментов завязаны на датах. А как известно, проблемы с часовыми поясами - это одна из главных проблем. С одной стороны, хочется хранить время в UTC времени, чтобы при смене часового поя

## Execution order

Помимо формирования списка задач, система должна уметь расставлять их по порядку выполнения. Алгоритм выстраивания задач:

1. Формируется список всех не выполненных задач (даже те, по которым дедлайн просрочен).
...
4. Когда задачи первого приоритета на данный день закончилась, алгоритм начинается смотреть задачи первого приоритета, которые на эту неделю назначены.
5. Когда задачи первого приоритета на день и неделю закончились, алгоритм смотрит задачи первого приоритета на месяц
6. Когда задачи первого приоритета закончились, система начинается добавлять в план задачи первого приоритета из очереди.
...
1. Система будет брать задачи, по которым прошёл дедлайн и пытаться их вставить в график. Есть вероятность, что задача было не выполнена ввиду того, что на неё просто не хватило времени и/или она была с низким приоритетом. Такие моменты лучше анализировать и переносить дедлайн (чтобы он не тянул остальные задачи дальше), повышать приоритет или осознавать, что у вас физически не достаточно времени на все задачи.
